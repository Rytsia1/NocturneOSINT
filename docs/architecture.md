...
Implemented so far as the Article ↔ Event provenance layer: `evidence (id, article_id, event_id, created_at, updated_at)`. A row records that an Article provides information about an Event. **Evidence records an association/provenance relationship. It does not independently establish that an Event is true or that an Article is reliable** — there is no confidence, credibility or verification field. Articles and Events are not coupled directly (no `articles.event_id` / `events.article_id`), so either can exist without the other and one Article can inform many Events and vice versa. The Source is reached through the Article (`articles.source_id`), not duplicated. Both foreign keys are `ON DELETE CASCADE`: a link is meaningless once either end is gone, so deleting an Article or an Event removes its Evidence and never leaves orphans (Source → Article stays `RESTRICT`). `UNIQUE (article_id, event_id)` prevents duplicate links; no relationship type is stored because the PRD defines none. Entity and Investigation links are deferred to their own steps.
